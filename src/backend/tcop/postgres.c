...
retry2:
	...
	if (Log_connections)
	{
		if (remote_port[0])
			ereport(LOG,
					(errmsg("rsocket connection established: host=%s port=%s",
							remote_host,
							remote_port)));
		else
			ereport(LOG,
					(errmsg("rsocket connection established: host=%s",
							remote_host)));
	}
