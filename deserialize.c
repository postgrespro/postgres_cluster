...
datum_deser(JsonbValue *var_value, bool typbyval)
{
	...
	JsonbValue var_key;
	var_key.type = jbvString;
	var_key.val.string.len = strlen("operargs");
	var_key.val.string.val = strdup("operargs");
	...
	if (var_value == NULL || var_value->type == jbvNull)
		local_node->operargs = NULL;
	else
		local_node->operargs = list_deser(var_value->val.binary.data, false);
